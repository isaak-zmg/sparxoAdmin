...
@observer
export default class ENUSDetails extends React.Component {
  constructor(props) {
    super(props)
    this.columns = [
      {
      ...
      },
      {
        ...
        render: (text, record, index) => {
          const { en_Details } = this.props.languageStore
          return (
            <ValueCell record={record} store={en_Details}  />
          )
        }
      }
    ]
  ...
  save = () => {
    ...
    var temp = toJS(languageStore.en_Details);
    var postData = {};
    for (var s in temp) {
  ...
  render() {
    const { en_Details } = this.props.languageStore;
    let datasource = toJS(en_Details);
